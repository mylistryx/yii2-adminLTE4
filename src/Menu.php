...
class Menu extends \yii\widgets\Menu
{
    ...
    protected function renderItems($items, bool $isSubmenu = false): string
    {
        $n = count($items);
        $lines = [];
        foreach ($items as $i => $item) {
            ...
            if (isset($item['divider']) && $item['divider']) {
                Html::removeCssClass($options, 'nav-item');
                Html::addCssClass($options, 'nav-divider');
            }
    ...
    protected function renderItem($item)
    {
        ...
        $treeFlag = '';
        if (isset($item['items'])) {
            $treeFlag = '<i class="nav-arrow fa fa-arrow-right"></i>';
        }
